...
public class Wheel : MonoBehaviour {
    ...
    void Awake() {
		...
		this.carTransform = origin.parent;
	...
	public bool checkGround() {
		...
		this.isGrounded = Physics.Raycast(this.transform.position, this.carTransform.transform.up * -1, out raycastResults, this.radius, 1<<LayerMask.NameToLayer("Map"));

		if(this.isGrounded) {
			this.mesh.localPosition = (raycastResults.point + this.carTransform.transform.up * this.radius) - this.transform.position;
		}
		else {
